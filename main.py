...
def sample_generator(x=20):
    x = (np.random.normal(loc=3.0, size=int(x/2)).tolist()) + (np.random.normal(loc=9.0, size=int(x/2)).tolist())
    y = (np.random.normal(loc=2.0, size=int(x/2)).tolist()) + (np.random.normal(loc=7.0, size=int(x/2)).tolist())
    label = [1 if i > int(x/2-1)  else 0 for i in range(x)]
    return x, y, label
