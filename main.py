...
def sample_generator(sample_size=20):
    x = (np.random.normal(loc=3.0, size=int(sample_size / 2)).tolist()) + (
        np.random.normal(loc=9.0, size=int(sample_size / 2)).tolist()
    )
    y = (np.random.normal(loc=2.0, size=int(sample_size / 2)).tolist()) + (
        np.random.normal(loc=7.0, size=int(sample_size / 2)).tolist()
    )
...
def user_imputs():
    ...
    print(
        "3. Probabilidade a partir da qual o algoritmo escolhe uma ou outra classe (default = 0.5)"
    )
    prob = input()
    if prob == "":
        no_compliance = False
...
plt.scatter(
    standard_data[:, 0], standard_data[:, 1], c=predicted_adjusted, marker="o", s=25
)
plt.plot(x, m)
plt.show()
